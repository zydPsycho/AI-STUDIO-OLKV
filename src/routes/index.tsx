...
function BloodLinkHome() {
  const [tab, setTab] = useState<"directory" | "profile" | "about">("directory");
  ...
  return (
    <div className="min-h-screen bg-background text-foreground pb-24">
      ...
      <main className="mx-auto max-w-6xl px-5 lg:px-10">
        {tab === "directory" && <DirectoryView donors={donors} filtered={filtered} query={query} group={group} loading={loading} onQuery={setQuery} onGroup={setGroup} onRefresh={loadDonors} onSelect={setSelected} onCreate={() => { setTab("profile"); window.scrollTo({ top: 0, behavior: "smooth" }); }} />}
        {tab === "profile" && <ProfileView form={form} myDonor={myDonor} saving={saving} message={message} error={error} onForm={updateForm} onPhoto={choosePhoto} onSubmit={publishProfile} onBack={() => setTab("directory")} />}
        {tab === "about" && <AboutView />}
      </main>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-20 border-t border-border bg-background/95 px-3 py-2 backdrop-blur sm:hidden">
        <div className="mx-auto grid max-w-md grid-cols-3 gap-2">
          ...
          <NavButton active={tab === "about"} icon={<Info className="size-5" />} label="About" onClick={() => setTab("about")} />
        </div>
      </nav>
...
function DirectoryView({ donors, filtered, query, group, loading, onQuery, onGroup, onRefresh, onSelect, onCreate }: { donors: Donor[]; filtered: Donor[]; query: string; group: string; loading: boolean; onQuery: (value: string) => void; onGroup: (value: string) => void; onRefresh: () => void; onSelect: (donor: Donor) => void; onCreate: () => void }) {
  const available = filtered.filter((donor) => donor.is_available).length;
  return <div className="space-y-8 py-5 lg:py-10">
    <section className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-end">
      <div><p className="mb-3 text-xs font-extrabold uppercase tracking-[0.2em] text-primary">KADU union directory</p><h1 className="max-w-2xl font-heading text-4xl font-extrabold leading-[1.04] tracking-tight sm:text-6xl">Find help close to home.</h1><p className="mt-5 max-w-xl text-base leading-7 text-muted-foreground sm:text-lg">Browse KADU donor members in Kavaratti by blood group. Names, ages, blood groups and phone numbers are available for union coordination.</p><button onClick={onCreate} className="mt-7 rounded-2xl bg-primary px-5 py-3.5 text-sm font-extrabold text-primary-foreground shadow-float transition hover:-translate-y-0.5"><Plus className="mr-2 inline size-4" />Add donor profile</button></div>
