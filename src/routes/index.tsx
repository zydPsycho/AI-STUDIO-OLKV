...
function BloodLinkHome() {
  ...
  async function loadAlerts(requiredBloodGroup?: string) {
    if (!requiredBloodGroup) { setAlerts([]); return; }
    const { data, error: readError } = await DB.from("kadu_emergency_alerts").select("id,sender_name,sender_phone,patient_name,admitted_in,emergency_type,required_blood_group,units_needed,notes,created_at").eq("is_active", true).eq("required_blood_group", requiredBloodGroup).order("created_at", { ascending: false }).limit(100);
  ...
  useEffect(() => { void loadAlerts(myDonor?.blood_group); }, [myDonor?.blood_group]);
  useEffect(() => {
  ...
  async function publishAlert(event: FormEvent<HTMLFormElement>) {
    ...
    else { if (myDonor?.blood_group === alertForm.required_blood_group) setAlerts((current) => [data as Alert, ...current]); setAlertForm(emptyAlertForm(myDonor)); setMessage(`Alert sent to ${alertForm.required_blood_group} donors in the KADU directory.`); }
 ...
 donors={donors} filtered={filtered} query={query} group={group} loading={loading} myDonor={myDonor} onQuery={setQuery} onGroup={setGroup} onRefresh={loadDonors} onSelect={setSelected} onCreate={() => { setTab("profile"); window.scrollTo({ top: 0, behavior: "smooth" }); }} onOpenMyProfile={() => { setTab("profile"); window.scrollTo({ top: 0, behavior: "smooth" }); }} />}
                {tab === "alerts" && <AlertsView alerts={alerts} form={alertForm} saving={alertSaving} message={message} error={error} onForm={(key, value) => setAlertForm((current) => ({ ...current, [key]: value }))} onSubmit={publishAlert} onSelect={setSelectedAlert} onRefresh={() => loadAlerts(myDonor?.blood_group)} onBack={() => setTab("directory")} />}
